...
class SecurityMonitor extends Thread
{
	...
	private float CurrentFireStatus = 1;	// Current fire status; 0=fire, 1=No fire;
	private float UserSprinklerStatus = 0;	// User updated Sprinkler status; 0=No, 1=Yes;
	private float CurrentSprinklerStatus = 0;	// Current Sprinkler status; 0=Off, 1=On;
	private boolean IsFireStatusUpdated = true;// These parameter signify the status of Security
	...
	public SecurityMonitor()
	{
		...
		try
		{
			...
			em = new EventManagerInterface("SMS Monitor", "Security monitoring system");
	...
	public SecurityMonitor( String EvmIpAddress )
	{
		...
		try
		{
			...
			em = new EventManagerInterface( EvtMgrIP, "SMS Monitor", "Security monitoring system" );
		}
	...
	public void run()
	{
		...
		int Delay15 = 15000;			// Delay 15 seconds
		...
		if (em != null)
		{
			...
			fi = new Indicator ("Fire status", mw.GetX()+ mw.Width(), 0);
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					if ( Evt.GetEventId() == 11 ) // Fire Alarm
					{
						try
						{
							CurrentFireStatus = Float.valueOf(Evt.GetMessage()).floatValue();
							IsFireStatusUpdated = true;
				...
				if (IsFireStatusUpdated){
					if (CurrentFireStatus == 0){
						fi.SetLampColorAndMessage("Monitor - Fire detected!", 1);
						mw.WriteMessage("Alert!! Fire Detected. Action needed!!");
						mw.WriteMessage("Sprinkler will be turned ON in 15 seconds.");
						mw.WriteMessage("Confirm or Cancel Sprinkler in console");
						
						try
						{
							Thread.sleep( Delay15 );

						} // try

						catch( Exception e )
						{
							System.out.println( "Sleep error 15 seconds:: " + e );

						} // catch
						
						if(UserSprinklerStatus == 0){
							PostEvent(12, "S1");
							CurrentSprinklerStatus = 1;
							mw.WriteMessage( "***Sprinkler turned ON automatically. ***" );
							
						}else{
							UserSprinklerStatus = 0;
						}


					}else{
						CurrentSprinklerStatus = 0;
						fi.SetLampColorAndMessage("Monitor - No Fire", 0);
						mw.WriteMessage("No Fire Detected by monitor");
						PostEvent(12, "S0");
					}
					
					IsFireStatusUpdated = false;
				}
	...
	***************************************************************************/
	...
	public void setUserSprinklerStatus(float status)
	{
		this.UserSprinklerStatus = 1;
		
		if (status == 0){
			PostEvent(12, "S1");
			this.CurrentSprinklerStatus = 1;
			mw.WriteMessage( "***Sprinkler turned ON by User. ***" );
		}else{
			PostEvent(12, "S0");
			this.CurrentSprinklerStatus = 0;
			mw.WriteMessage( "***Sprinkler turned OFF by User. ***" );
		}
	...
	***************************************************************************/
	...
	public float getCurrentFireStatus()
	{
	...
	***************************************************************************/
	...
	public float getCurrentSprinklerStatus()
	{
		return CurrentSprinklerStatus;
	...
	***************************************************************************/
	...
	public void turnOffSprinkler()	{
		this.CurrentSprinklerStatus = 0;
		PostEvent(12, "S0");
		mw.WriteMessage( "***Sprinkler found on. Turned OFF by User. ***" );
	...
	***************************************************************************/
	...
	private void PostEvent(int eventId, String postAlarm )
	{
		// Here we create the event.
