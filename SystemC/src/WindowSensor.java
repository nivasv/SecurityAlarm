...
class WindowSensor extends Thread
{
	...
	public WindowSensor()
	{
		...
		try
		{
			...
			em = new EventManagerInterface("Window Sensor", "To check the window status");
	...
	public WindowSensor( String EvmIpAddress )
	{
		...
		try
		{
			...
			em = new EventManagerInterface( EvtMgrIP, "Window Sensor", "To check the window status" );
		}
...
//		boolean OFF = false;			// Used to turn off heaters, chillers, humidifiers, and dehumidifiers
		...
		if (em != null)
		{
			...
			mw = new MessageWindow("Window Sensor Console", 0, 0);
	...
	***************************************************************************/
	...
	static private void PostStatus(EventManagerInterface ei, float status)
	{
		...
		Event evt = new Event( (int) 6, String.valueOf(status) );
