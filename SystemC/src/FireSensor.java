...
class FireSensor extends Thread
{
	...
	private float SensorStatus = 0;		// These parameter signify the status of Sensor; 1 = ON, 0 = OFF
	private boolean IsStatusUpdated = false;// These parameter signify the status of Sensor
	...
	public FireSensor()
	{
		...
		try
		{
			...
			em = new EventManagerInterface("Fire Sensor", "To check the fire");

		}

		catch (Exception e)
		{
			System.out.println("WindowSensor::Error instantiating event manager interface: " + e);
			Registered = false;
	...
	public FireSensor( String EvmIpAddress )
	{
		...
		try
		{
			...
			em = new EventManagerInterface( EvtMgrIP, "Fire Sensor", "To check the fire" );
		}
...
//		int EvtId = 0;					// User specified event ID
		...
		int	Delay = 4000;				// The loop delay (1 second)
...
//		boolean OFF = false;			// Used to turn off heaters, chillers, humidifiers, and dehumidifiers
		...
		if (em != null)
		{
			...
			mw = new MessageWindow("Fire Sensor Console", 0, 0);
...
/*					if (SensorStatus == 0){
						mw.WriteMessage("Window is Open/Broken" );
					}else{
						mw.WriteMessage("Window is closed" );
					}
*/
	...
	***************************************************************************/
	...
	public void SetSensorStatus(float status)
	{
		...
		if (status == 1){
			mw.WriteMessage( "*** Sensor status : No Fire ***" );
		}
		else{
			mw.WriteMessage( "*** Sensor status : Fire is detected  ***" );
		}
	...
	***************************************************************************/
	...
	static private void PostStatus(EventManagerInterface ei, String status)
	{
		// Here we create the event.

		Event evt = new Event( (int) 10, status );
