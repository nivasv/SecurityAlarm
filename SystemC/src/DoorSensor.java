...
class DoorSensor extends Thread
{
	...
	public DoorSensor()
	{
		...
		try
		{
			...
			em = new EventManagerInterface("Door Sensor", "To check the door status");

		}

		catch (Exception e)
		{
			System.out.println("DoorSensor::Error instantiating event manager interface: " + e);
			Registered = false;
	...
	public DoorSensor( String EvmIpAddress )
	{
		...
		try
		{
			...
			em = new EventManagerInterface( EvtMgrIP,"Door Sensor", "To check the door status" );
		}
...
//		boolean OFF = false;			// Used to turn off heaters, chillers, humidifiers, and dehumidifiers
		...
		if (em != null)
		{
			...
			mw = new MessageWindow("Door Sensor Console", 0, 0);
	//		wi = new Indicator ("WINDOW SENSOR", mw.GetX()+ mw.Width(), 0);
			...
			while ( !Done )
			{
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					if ( Evt.GetEventId() == 3 )
					{
						...
						{
							AlarmState = true;
							mw.WriteMessage("Door Sensor is ON.");
						...
						{
							AlarmState = false;
							mw.WriteMessage("Door Sensor is OFF.");
		...
		//				wi.dispose();
				...
				if (AlarmState && IsStatusUpdated) {
					...
					if (SensorStatus == 0){
						mw.WriteMessage("Door is Open/Broken" );
					}else{
						mw.WriteMessage("Door is closed" );
					}
	...
	***************************************************************************/
	...
	static private void PostStatus(EventManagerInterface ei, float status)
	{
		...
		Event evt = new Event( (int) 7, String.valueOf(status) );
		...
		{
			System.out.println( "Error Posting door status :: " + e );
