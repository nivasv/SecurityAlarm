...
class FireAlarmController
{
	public static void main(String args[])
	{
		...
		boolean FireAlarmState = false;	// Window alarm state: false == off, true == on
		...
		/////////////////////////////////////////////////////////////////////////////////
 		...
 		if ( args.length == 0 )
 		{
			...
			try
			{
				...
				em = new EventManagerInterface("Fire Alarm Controller", "To alarm on fire");
			}
			...
			try
			{
				...
				em = new EventManagerInterface( EvtMgrIP, "Fire Alarm Controller", "To alarm on fire" );
			}
		...
		if (em != null)
		{
			...
			*/
			...
			Indicator fi = new Indicator ("Fire Alarm Controller", mw.GetX(), mw.GetY()+mw.Height());
		//	Indicator ti = new Indicator ("Alarm Controller OFF", mw.GetX()+(wi.Width()*2), mw.GetY()+mw.Height());
			...
			mw.WriteMessage("No Fire detected by Fire Alarm" );
			fi.SetLampColorAndMessage("FIRE ALARM OFF", 3);
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					{
						if (Evt.GetMessage().equalsIgnoreCase("0")) // Window alarm on
						{
							FireAlarmState = true;
							mw.WriteMessage("Fire detected by Fire Alarm. Alert user!!!" );
							fi.SetLampColorAndMessage("FIRE ALARM ON", 1);
							PostStatus(em, 0);

						} // if

						if (Evt.GetMessage().equalsIgnoreCase("1")) // Window alarm off
						{
